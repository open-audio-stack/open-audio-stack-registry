...
The registry root (`GET /`) and the four type-listing endpoints (`GET /apps`, `GET /plugins`, `GET /presets`, `GET /projects`) enumerate every package of that type in one document, so to keep them lightweight they each only summarize the **latest** version of every package. Each file in that version's `files` array is still present - with its `architectures`, `systems`, `contains`, `type`, `size`, and (when set) `downloads`/`attested` - so filtering and sorting a listing by any of those stays fully accurate, but `url` and `sha256` are omitted from each file, since nothing needs them until a package is actually installed. Everything below that — org (`GET /apps/{org}`), package (`GET /apps/{slug}`), and version (`GET /apps/{slug}/{version}`) endpoints — still return the complete version history, and every file's `url` and `sha256`, for every version. Clients that need older versions or that are about to download a file must fetch one of those narrower endpoints.
...
`apps`, `plugins`, `presets` and `projects` here are the same latest-version-only documents (each file's `url`/`sha256` omitted) as their respective list endpoints - see [Listing endpoints vs package endpoints](#listing-endpoints-vs-package-endpoints).
...
| Field     | Type                | Description                                                                                                                                                                                                                                                                                                            | Example                            |
| :-------- | :------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------- |
| downloads | number              | Optional. Rollup of every version's `downloads` - i.e. total downloads across all published versions of this package, all-time. Computed during registry build; omitted when zero.                                                                                                                                     | `2502`                             |
...
| versions  | \[version: string\] | Package versions Object. Contains every released version at the org/package/version endpoints; trimmed to just the latest version at the registry root and list endpoints, with each file's `url`/`sha256` also omitted there - see [Listing endpoints vs package endpoints](#listing-endpoints-vs-package-endpoints). | `"versions": { "1.3.1": { ... } }` |
...
| downloads   | number           | Optional. Rollup of this version's file download counts (sum of each file's `downloads`). Computed during registry build; omitted when zero.                                                                                                                                                                                                                                                                                                                                        | `2413`                                                                                                                             |
...
Latest version only; each file's `url`/`sha256` also omitted - see [Listing endpoints vs package endpoints](#listing-endpoints-vs-package-endpoints).

```
GET /apps
{
  "free-audio/clapinfo": { "slug": "free-audio/clapinfo", "version": "1.2.2", "versions": { "1.2.2": { ... } } },
}
```
...
GET /plugins
{
  "surge-synthesizer/surge": { "slug": "surge-synthesizer/surge", "version": "1.3.1", "versions": { "1.3.1": { ... } } },
}
```
...
GET /projects
{
  "kmt/banwer": { "slug": "kmt/banwer", "version": "1.0.1", "versions": { "1.0.1": { ... } } },
}
```
...
GET /presets
{
  "jh/floating-rhodes": { "slug": "jh/floating-rhodes", "version": "2.2.0", "versions": { "2.2.0": { ... } } },
}
```
...
| sha256        | string                                                 | File checksum for security validation that the published file matches the downloaded file. Omitted at the registry root and list endpoints - see [Listing endpoints vs package endpoints](#listing-endpoints-vs-package-endpoints).                                                                                              | `"3af35f02121a3c7ad6375bfe9c29a382f5882fa443bbf4c2df347a255c0edf2a"`                                                 |
...
| url           | string                                                 | File download url (https). Files can be hosted anywhere on the web, but we recommend using GitHub Releases because it provides auditability of who changed the files and when. Omitted at the registry root and list endpoints - see [Listing endpoints vs package endpoints](#listing-endpoints-vs-package-endpoints).          | `"https://github.com/surge-synthesizer/releases-xt/releases/download/1.3.1/surge-xt-linux-1.3.1-pluginsonly.tar.gz"` |
