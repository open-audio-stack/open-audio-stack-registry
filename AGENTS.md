...
**GitHub as a mirror, not the release channel**: some projects (Flathub apps, AppStream-catalogued Linux software, plugins with their own vendor site) treat GitHub purely as a source mirror and always will — official binaries live on Flathub, in distro repos, or on the author's own site instead. A missing GitHub release here isn't a temporary gap the maintainer just hasn't gotten to; it's how the project is permanently distributed. Signs to check before assuming this is fixable: a README/site that says "official builds are at ...", an AppStream/`metainfo.xml` file in the repo pointing at a different distribution channel, or a repo description calling itself a mirror. If confirmed, this is a 3f exclusion (see below), not a "few small changes" ask — filing an issue asking the maintainer to add GitHub releases they have no intention of adding just burns a tracking issue on a dead end.
...
- **Content type invalid** — a library/framework/source-module meant for embedding (even if some third-party list describes it as "a plugin"), a DAW or full application, a research/training repo with no packaged distributable, or a browser-only tool with no installable artifact at all (no systems/binary concept applies). Read past the one-line description; check what the README actually says the project _is_ and how it's meant to be used. The README's own words are usually the tell — treat any of these phrases as a strong signal to stop and re-read the whole README before filing, not just skim past them: "learning resource," "code example(s)," "experimental," "thesis"/"demo," "proof of concept." A repo that describes itself this way is telling you directly it isn't meant for general use, regardless of what license/release/image status it happens to have.
- **Fork exists only to feed a bundler/host project** — a fork whose own description says it exists to pull upstream code into a different host app/bundle (e.g. "downstream fork to aid in bundling into X") has no intention of shipping its own releases or images, even if it welcomes contributions. This isn't quite the same as "content type invalid" (the underlying plugin format may be real) — it's that _this particular repo_ isn't the place a release will ever come from.
